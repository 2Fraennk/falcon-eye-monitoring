...
def test_dashboard_exists(playwright: Playwright) -> None:
    ...
    expect(page.get_by_role("link", name="demoapp")).to_be_visible(60000)
...
def test_dashboard_contains_panels(playwright: Playwright) -> None:
    ...
    for panel_heading in ["trace duration",
        ...
        expect(panel).to_be_visible(60000)
...
def test_panel_data_trace_duration_graph(playwright: Playwright) -> None:
    ...
    expect(page.get_by_test_id("data-testid VizLegend series Duration")
           .get_by_role("button", name="Duration")).to_be_visible(60000)
...
def test_panel_data_trace_duration_list(playwright: Playwright) -> None:
    ...
    expect(page.get_by_text("demoapp: GET /owners").first).to_be_visible(60000)
    expect(page.get_by_text("demoapp: GET /vets.html").first).to_be_visible(60000)
    expect(page.get_by_text("demoapp: GET /oups").first).to_be_visible(60000)
...
def test_panel_data_runtime_exception(playwright: Playwright) -> None:
    ...
    expect(page.get_by_test_id("data-testid Panel header logfile - exceptions")
           .locator("div").filter(has_text="exception").first).to_be_visible(60000)
...
def test_panel_data_http_status(playwright: Playwright) -> None:
    ...
    expect(page.get_by_test_id("data-testid Panel header http status code")
           .locator("div").filter(has_text="Name").first).to_be_visible(60000)
...
def test_panel_data_app_starts(playwright: Playwright) -> None:
    ...
    expect(page.get_by_test_id("data-testid Panel header logfile - app start duration")
           .get_by_role("button", name="container").first).to_be_visible(60000)
